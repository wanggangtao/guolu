...
try {
    ...
    $result = project_inspectlog::getPageList($page,$pageSize,1,$projectId,"",0,0,$content);

    $resData["total"] = project_inspectlog::getPageList($page,$pageSize,0,$projectId,"",0,0,$content);
    ...
    if(!empty($result))
    {
        ...
        {
            $dataItem = array();
            $dataItem["surveyid"] = $item["id"];
            $dataItem["company"] = $item["company"];
            $dataItem["surveytime"] = date("Y-m-d H:i:s",$item["inspecttime"]);
