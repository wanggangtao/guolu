...
try {
    ...
    if(($projectinfo['one_status'] != 2 && $projectinfo['one_status'] != 3) || ($projectinfo['two_status'] != 2 && $projectinfo['two_status'] != 3) || ($projectinfo['three_status'] != 2 && $projectinfo['three_status'] != 3)){
        throw new MyException("低星级还未提交，请先提交低星级!",1171);
    }
    if(empty($id)){//首次提交
        throw new MyException("还未保存，请先保存!",1172);
    }else{//提交前保存过
