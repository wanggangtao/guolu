...
switch($type){
    ...
    case 'guoluvideodetail'://锅炉详细视频
        ...
        $filepath_rel    = 'userfiles/upload/guludetailvideo/';//相对路径
        $filepath_abs    = $FILE_PATH.$filepath_rel;//绝对路径
        ...
        try{
            $fup = new FileUpload('500M', $allowext);
            $r = $fup->upload($fileElement, $filepath_abs, '', true);
            $name_abs = $filepath_abs.$r;
            $name_rel = $filepath_rel.$r;
            //上传成功
    ...
    case 'guoluvideoplacehold'://锅炉占位图
        $allowext        = array('png','gif','jpg','jpeg');
        $fileElement     = 'placehold_input';
        $filepath_rel    = 'userfiles/upload/guoluvideoplacehold/';//相对路径
        $filepath_abs    = $FILE_PATH.$filepath_rel;//绝对路径
        if (!file_exists($filepath_abs)) {
            mkdir($filepath_abs, 0777,true);
            chmod($filepath_abs, 0777);
        ...
        try{
            $fup = new FileUpload('1M', $allowext);
            $r = $fup->upload($fileElement, $filepath_abs, '', true);
            $name_abs = $filepath_abs.$r;
            $name_rel = $filepath_rel.$r;
            //上传成功
    ...
    case 'logoimg'://锅炉占位图
        $allowext        = array('png','gif','jpg','jpeg');
        $fileElement     = 'placehold_input';
        $filepath_rel    = 'userfiles/upload/logoimg/';//相对路径
        $filepath_abs    = $FILE_PATH.$filepath_rel;//绝对路径
        if (!file_exists($filepath_abs)) {
            mkdir($filepath_abs, 0777,true);
            chmod($filepath_abs, 0777);
    ...
    case 'guoludetail'://锅炉详细图片
        ...
        $filepath_rel    = 'userfiles/upload/guludetailimg/';//相对路径
        $filepath_abs    = $FILE_PATH.$filepath_rel;//绝对路径
        if (!file_exists($filepath_abs)) {
            mkdir($filepath_abs, 0777,true);
            chmod($filepath_abs, 0777);
