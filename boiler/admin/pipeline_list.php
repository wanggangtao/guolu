...
//初始化
$count     = Pipeline_attr::getList(1, 15, 1, $vender);
$pageSize  = 15;
$pagecount = ceil($count / $pageSize);
$page      = getPage($pagecount);
$rows      = Pipeline_attr::getList($page, $pageSize, 0, $vender);
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#search').click(function(){
                ...
                location.href  = "pipeline_list.php?vender="+vender;
            });
            //添加
            $('#add').click(function(){
                layer.open({
            ...
            $(".editinfo").click(function(){
                ...
                layer.open({
                    ...
                    shade: 0.3,
                    area: ['800px', '350px'],
                    content: 'pipeline_edit.php?id='+thisid
                });
            });
            //详情
            $(".info").click(function(){
...
<div id="container">
    ...
    <div id="maincontent">
        <div id="position">当前位置：<a href="guolu_list.php">产品中心</a> &gt; 产品管理 &gt; 辅机 &gt; 管道泵</div>
        ...
        <div class="tablelist">
            <table>
                ...
                if(!empty($rows)){
                    foreach($rows as $row){
                        $venderInfo = Dict::getInfoById($row['pipeline_vender']);
                        echo '<tr>
                                <td class="center">'.$nums.'</td>
                                <td class="center">'.$row['pipeline_version'].'</td>
                                <td class="center">'.($venderInfo?$venderInfo['name']:'').'</td>
                                <td class="center">'.floatval($row['pipeline_flow_min']).' / '.floatval($row['pipeline_flow_mid']).' / '.floatval($row['pipeline_flow_max']).'</td>
                                <td class="center">'.floatval($row['pipeline_lift_min']).' / '.floatval($row['pipeline_lift_mid']).' / '.floatval($row['pipeline_lift_max']).'</td>
                                <td class="center">
                                    ...
                                    <input type="hidden" id="aid" value="'.$row['pipeline_id'].'"/>
