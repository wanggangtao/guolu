...
//初始化
$count     = Service_record::getCount($params);
$pageSize  = 10;
...
$rows      = Service_record::getList($params);
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#search').click(function(){
                ...
                location.href  = "weixin_service_record.php?key="+key+"&&starttime="+startTime+"&&endtime="+endtime;
            });
            //添加
            $('#add').click(function(){
                layer.open({
...
<div id="container">
    ...
    <div id="maincontent">
        ...
        <div id="handlelist">
            <input type="text" class="text-input input-length-10" id="key" value="<?php echo $params['value']; ?>" >
            ...
            <label> ~ </label>
            <input type="text" class="laydate-icon input-length-30"  name="stoptime" id="endtime" value="<?php echo $endtime ?>" />
            ...
            <table>
                ...
                if(!empty($rows)){
                    ...
                    foreach($rows as $row){
                        ...
                        $brand="暂无";
                        $user_type="游客";
                        $user_info=User_account::getAllByOpenid($row['user_openid']);
                        if($user_info){
                            $user_type="注册用户";
                            if($row['community']){
                        ...
                        echo '<tr>
                                ...
                                <td class="center">'.$community.'</td>
                                <td class="center">'.$brand.'</td>
                                <td class="center">'.$user_type.'</td>
                                <td class="center">'.date("Y-m-d H:i:s",$row['time']).'</td>
                                <td class="center">
                                    <a class="view_record" href="javascript:void(0)">聊天记录</a>
                                    <a class="delete" href="javascript:void(0)">删除</a>
                                    <input type="hidden" id="cid" value="'.$row['user_openid'].'"/>
                                </td>
                            </tr>';
                        $i++;
                    }
