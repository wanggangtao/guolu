...
class Table_factory extends Table {
    ...
    static public function getInfoById($fid){
        ...
        $fid = $mypdo->sql_check_input(array('number', $fid));

        $sql = "select * from ".$mypdo->prefix."case_tpl where tpl_id = $fid limit 1";
        $rs  = $mypdo->sqlQuery($sql);
        $r   = array();
        if($rs){
            foreach($rs as $key => $val){
