...
class Table_repair_order extends Table {
     ...
     * @return mixed
     * wanggangtao
     * 师傅端提交待审核订单，订单状态变为待审核
     */
    static public function master_remark($order_id,$remark,$pay_style)
    {
        global $mypdo;
        $order_id = $mypdo->sql_check_input(array('number', $order_id));
        $param = array(
            'order_remarks'=>array('string', $remark),
            'order_repair_pay_style'=>array('number', $pay_style),
            'order_status'=>array('number', 3),//待审核
            'order_child_status'=>array('number', 32),//第二状态待审核
        );
        $where = array(
            "order_id" => array('number', $order_id)
        );
        //返回结果
...
//        echo $sql;
    ...
    static public function getList_repair_master($master_id,$status ,$child_status){
        global $mypdo;
        $master_id = $mypdo->sql_check_input(array('number', $master_id));//维修师傅的ID
...
//        var_dump($r);
    ...
    static public function valid_order($id,$reason)
    {
        ...
        $param = array(//用户端    取消订单，订单状态直接变为以完成
            'order_status'=>array('number', 3),//第一状态已取消
            'order_child_status'=>array('number', 31),//第二状态已取消
            'order_client_valid'=>array('string', $reason),//原因
            'order_content'=>array('string',$date ),//日期
        );
        $where = array(
            "order_id" => array('number', $id)
        );
